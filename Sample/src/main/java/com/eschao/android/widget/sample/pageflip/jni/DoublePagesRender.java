...
public class DoublePagesRender extends PageRender {
    ...
    public void onDrawFrame() {
        ...
        if (!PageFlipLib.isFirstTextureSet(true)) {
            drawPage(PageFlipLib.isLeftPage() ? mPageNo : mPageNo + 1);
    ...
    public void onSurfaceChanged(int width, int height) {
        ...
        int pageW = (int)PageFlipLib.getPageWidth(true);
        int pageH = (int)PageFlipLib.getPageHeight();
    ...
    public boolean onEndedDrawing(int what) {
        if (what == DRAW_ANIMATING_FRAME) {
            ...
            // animation is finished
                ...
                // page mode
                if (PageFlipLib.getFlipState() == PageFlipLib.END_WITH_FORWARD)
                {
                    //final Page first = mPageFlip.getFirstPage();
                    //final Page second = mPageFlip.getSecondPage();
                    PageFlipLib.swapTexturesWith(true);
